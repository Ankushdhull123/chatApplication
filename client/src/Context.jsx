...
export const MyProvider = ({ children }) => {
    ...
    async function fetchData() {
      try {
        const res = await fetch('https://chatapplication-jxtt.onrender.com/allusers');
 ...
 useEffect(() => {
  const fetchFollowings = async () => {
    try {
      const res = await fetch(`https://chatapplication-jxtt.onrender.com/user/${logindata.id}/followings`);
      if (!res.ok) {
